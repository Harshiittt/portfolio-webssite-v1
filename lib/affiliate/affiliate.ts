...
export function amazonAffiliate(url: string): string {
  if (!AMAZON_TAG) return url; // ← guard
  try {
    const u = new URL(url);
...
function flipkartAffiliate(url: string): string {
  if (!FLIPKART_ID) return url; // ← guard
...
function meeshoAffiliate(url: string): string {
  const sid = process.env.MEESHO_AFFILIATE_ID ?? "";
  if (!sid) return url; // ← guard
  const params = new URLSearchParams({ sid, url: encodeURIComponent(url) });
  return `https://vcm.vcommission.com/track?${params.toString()}`;
}
...
function ajioAffiliate(url: string): string {
  const sid = process.env.AJIO_AFFILIATE_ID ?? "";
  if (!sid) return url; // ← guard
  const params = new URLSearchParams({ sid, url: encodeURIComponent(url) });
  return `https://vcm.vcommission.com/track?${params.toString()}`;
}

function nykaaAffiliate(url: string): string {
  const sid = process.env.NYKAA_AFFILIATE_ID ?? "";
  if (!sid) return url; // ← guard
  ...
  return `https://www.nykaa.com/affiliate?${params.toString()}`;
}
...
export function attachAffiliate(url: string, source: string): string {
  ...
  switch (source.toLowerCase()) {
    ...
    case "ajio":   affiliateUrl = ajioAffiliate(url); break;
    case "nykaa":  affiliateUrl = nykaaAffiliate(url); break;
